// ==========================================
// THPS WIDGET: MEDIUM UPLOAD SPEECH
// Isolated DOM structure & Vertically stacked layout
// ==========================================

class ThpsUploadSpeechMed extends HTMLElement {
    connectedCallback() {
        this.innerHTML = `
            <div class="glass-panel p-5 sm:p-6 rounded-2xl border-t-4 border-emerald-500 shadow-sm flex flex-col bg-white relative w-full h-full min-h-[300px] group">
                ...
                <div class="flex justify-between items-start mb-4">
                    <div>
                        <h3 class="text-xs font-bold text-slate-500 uppercase tracking-wider">Upload Speech</h3>
                        <p class="text-[10px] text-slate-400 mt-0.5">Snip media or restore a session</p>
                    </div>
                </div>
                
                <div class="med-status-banner hidden w-full text-xs font-bold p-3 rounded-lg mb-4 text-center transition-all"></div>

                <div class="med-drop-zone-wrapper w-full flex flex-col gap-2 flex-1">
                    <div class="med-drop-zone w-full flex-1 min-h-[140px] border-2 border-dashed border-slate-300 rounded-xl flex flex-col items-center justify-center bg-slate-50 hover:bg-slate-100 hover:border-emerald-400 transition-all cursor-pointer relative">
                        <input type="file" class="med-file-input absolute inset-0 w-full h-full opacity-0 cursor-pointer" accept="audio/*,video/mp4,video/webm,application/json,.json">
                        <i data-lucide="upload-cloud" class="w-8 h-8 text-slate-400 mb-2 pointer-events-none"></i>
                        <span class="text-sm font-bold text-slate-600 pointer-events-none text-center">Drag & Drop<br/>Media or Data</span>
                    </div>
                    
                    <button class="med-btn-export-session w-full mt-2 py-2.5 bg-slate-50 hover:bg-slate-100 text-slate-500 hover:text-slate-700 rounded-xl text-xs font-bold uppercase tracking-widest transition-all flex items-center justify-center gap-2 border border-slate-200 shadow-sm relative z-10 active:scale-95">
                        <i data-lucide="download" class="w-4 h-4"></i> Get Session Data
                    </button>
                </div>

                <div class="med-edit-panel hidden w-full flex-col gap-3">
                    
                    <div class="flex items-center justify-between bg-slate-50 p-2.5 rounded-lg border border-slate-200">
                        <div class="flex items-center gap-2 overflow-hidden pr-2">
                            <i data-lucide="file-audio" class="w-4 h-4 text-emerald-500 shrink-0"></i>
                            <span class="med-file-name-display text-[10px] font-bold text-slate-700 truncate">filename.mp3</span>
                        </div>
                        <button class="med-btn-reset-file text-[9px] font-bold text-slate-500 hover:text-rose-500 uppercase tracking-widest px-2 py-1 bg-white border border-slate-200 rounded shadow-sm shrink-0">Replace</button>
                    </div>

                    <div class="w-full bg-slate-800 rounded-xl p-2 shadow-inner">
                        <audio class="med-audio-player w-full h-7 outline-none" controls></audio>
                    </div>

                    <!-- Vertical Stacked IN/OUT panels for narrower medium widget -->
                    <div class="flex flex-col gap-2">
                        <div class="flex flex-col gap-1.5 p-2 bg-slate-50 border border-slate-200 rounded-lg">
                            <div class="flex justify-between items-center">
                                <span class="text-[9px] font-bold text-slate-500 uppercase tracking-widest">Start (IN)</span>
                                <span class="med-val-in-time text-[10px] font-mono font-bold text-slate-800">0.0s</span>
                            </div>
                            <button class="med-btn-set-in w-full py-1.5 bg-white border border-slate-300 rounded text-[9px] font-bold text-slate-600 hover:border-emerald-500 hover:text-emerald-600 transition-colors shadow-sm">Set to Current Time</button>
                        </div>
                        <div class="flex flex-col gap-1.5 p-2 bg-slate-50 border border-slate-200 rounded-lg">
                            <div class="flex justify-between items-center">
                                <span class="text-[9px] font-bold text-slate-500 uppercase tracking-widest">End (OUT)</span>
                                <span class="med-val-out-time text-[10px] font-mono font-bold text-slate-800">End</span>
                            </div>
                            <button class="med-btn-set-out w-full py-1.5 bg-white border border-slate-300 rounded text-[9px] font-bold text-slate-600 hover:border-emerald-500 hover:text-emerald-600 transition-colors shadow-sm">Set to Current Time</button>
                        </div>
                    </div>

                    <!-- Vertically Stacked Action Buttons -->
                    <div class="flex flex-col gap-2 mt-2">
                        <button class="med-btn-download-snip w-full py-2.5 bg-white border-2 border-slate-200 text-slate-600 rounded-xl text-[10px] font-bold uppercase tracking-widest hover:border-blue-400 hover:text-blue-600 transition-all flex items-center justify-center gap-2 shadow-sm">
                            <i data-lucide="music" class="w-3.5 h-3.5 shrink-0"></i> <span>Download Snip</span>
                        </button>
                        <button class="med-btn-export-snip-data w-full py-2.5 bg-white border-2 border-slate-200 text-slate-600 rounded-xl text-[10px] font-bold uppercase tracking-widest hover:border-indigo-400 hover:text-indigo-600 transition-all flex items-center justify-center gap-2 shadow-sm">
                            <i data-lucide="file-json" class="w-3.5 h-3.5 shrink-0"></i> <span>Get Snip Data</span>
                        </button>
                        <button class="med-btn-analyze-snip w-full py-2.5 bg-emerald-600 text-white rounded-xl text-[10px] font-bold uppercase tracking-widest hover:bg-emerald-500 transition-all flex items-center justify-center gap-2 shadow-md">
                            <i data-lucide="cpu" class="w-3.5 h-3.5 shrink-0"></i> <span>Analyze Snip</span>
                        </button>
                    </div>
                </div>
        ...
        this.audioPlayer = this.querySelector('.med-audio-player');
        this.fileInput = this.querySelector('.med-file-input');
        this.dropZone = this.querySelector('.med-drop-zone');
        this.dropZoneWrapper = this.querySelector('.med-drop-zone-wrapper');
        this.editPanel = this.querySelector('.med-edit-panel');
        this.statusBanner = this.querySelector('.med-status-banner');
    ...
    setupEventListeners() {
        ...
        this.querySelector('.med-btn-export-session').addEventListener('click', () => this.exportSession());

        // Reset File
        this.querySelector('.med-btn-reset-file').addEventListener('click', () => {
            this.audioBuffer = null;
        ...
        // Snipping Controls
        this.querySelector('.med-btn-set-in').addEventListener('click', () => {
            ...
            this.querySelector('.med-val-in-time').innerText = this.inTime.toFixed(1) + 's';
        });

        this.querySelector('.med-btn-set-out').addEventListener('click', () => {
            ...
            this.querySelector('.med-val-out-time').innerText = this.outTime.toFixed(1) + 's';
        });

        // Action Buttons
        this.querySelector('.med-btn-download-snip').addEventListener('click', () => this.downloadSnip());
        this.querySelector('.med-btn-export-snip-data').addEventListener('click', () => this.exportSession());
        this.querySelector('.med-btn-analyze-snip').addEventListener('click', () => this.analyzeSnip());
    }
    ...
    async handleFile(file) {
        ...
        this.querySelector('.med-file-name-display').innerText = file.name;
        ...
        try {
            ...
            this.querySelector('.med-val-in-time').innerText = '0.0s';
            this.querySelector('.med-val-out-time').innerText = this.outTime.toFixed(1) + 's';
...
customElements.define('thps-upload-speech-med', ThpsUploadSpeechMed);
