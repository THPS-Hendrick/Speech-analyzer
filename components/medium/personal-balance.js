// ==========================================
// THPS WIDGET: MEDIUM PERSONAL BALANCE
// ==========================================

class ThpsPersonalBalance extends HTMLElement {
    connectedCallback() {
        this.innerHTML = `
    ...
    update(data) {
        ...
        const personalPercent = data.personal;
        ...
        valEl.textContent = `${personalPercent}%`;
        let pColor = personalPercent <= 29 ? "#94a3b8" : personalPercent <= 44 ? "#10b981" : personalPercent <= 60 ? "#00B700" : "#a855f7";
        intEl.textContent = personalPercent <= 29 ? "Highly Detached" : personalPercent <= 44 ? "Good Balance" : personalPercent <= 60 ? "Highly Personal" : "Overly Conversational?";
        
        card.style.borderLeftColor = pColor;
        prog.style.backgroundColor = pColor;
        valEl.style.color = pColor;
        prog.style.width = `${Math.min(100, Math.max(0, personalPercent))}%`;
    }
}
customElements.define('thps-personal-balance', ThpsPersonalBalance);
