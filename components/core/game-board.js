...
class ThpsGameBoard extends HTMLElement {
    ...
    render() {
        ...
        const VERSION_TAG = "v.01:04:00 ACST";
    ...
    setGameState(newState, finalScoreNum = 0) {
        ...
        const title = this.querySelector('#gb-board-title');
        const starPanel = this.querySelector('#gb-board-star-panel');
        ...
        if (newState === 'PLAYING') {
            ...
            // Fade out the initial icons and fade in the loaders
            ...
            ['challenge', 'sponsor', 'script', 'micCheck'].forEach((key) => {
                ...
                if (cardEl) {
                    ...
                    if (backFace) {
                        // Apply the Zelda Glow class
                        backFace.className = 'thps-chest-bg absolute inset-0 w-full h-full backface-hidden rounded-lg md:rounded-xl shadow-md md:shadow-xl border-2 md:border-4 transition-all duration-1000 bg-slate-900 border-amber-400/50 shadow-[0_0_15px_rgba(251,191,36,0.3)] animate-zelda-glow overflow-hidden';
                    }
                }
            });
    ...
    resetBoardToIdle() {
        ...
        cardData.forEach(c => {
            ...
            if (cardEl) {
                const backFace = cardEl.querySelector('.thps-chest-bg');
                if (backFace) {
                    backFace.className = `thps-chest-bg absolute inset-0 w-full h-full backface-hidden rounded-lg md:rounded-xl shadow-md md:shadow-xl border-2 md:border-4 transition-all duration-500 overflow-hidden`;
                    if (c.special) {
                        backFace.classList.add('bg-gradient-to-br', 'from-red-800', 'via-red-900', 'to-black', 'border-amber-400');
                    } else {
                        backFace.classList.add(c.bg, 'border-white');
                    }
                    ...
                    if (resultsEl) {
                        resultsEl.innerHTML = ''; // Wipe old scores
                        resultsEl.classList.add('opacity-0');
                    }
                }
                
                // Put cards back to their proper face up/down status based on difficulty level
    ...
    update(data) {
        // ONLY accept update if we are actively expecting it (prevents race conditions)
        ...
        // Un-pulse the Zelda chest glow immediately
